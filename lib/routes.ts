...
export const routes: RouteItem[] = [
  { path: "/", nav: false, label: { en: "Home", ru: "Главная", hy: "Գլխավոր", es: "Inicio", pt: "Inicio" } },
  ...
  { path: "/contact", nav: false, label: { en: "Contact", ru: "Контакты", hy: "Կապ", es: "Contacto", pt: "Contato" } }
];
...
export function getLocalizedHomePath(locale: Locale, path: string): string {
  ...
  if (localeReadyPaths.includes(path)) return getLocalizedPath(locale, path);
  return path;
}
...
export function stripLocaleFromPath(pathname: string): string {
  ...
  if (["ru", "hy", "es", "pt"].includes(first)) return rest.length ? `/${rest.join("/")}` : "/";
  return pathname;
}
