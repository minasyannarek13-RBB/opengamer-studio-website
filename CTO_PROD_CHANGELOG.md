...
- **Purpose:** correct the prior locale-routing assumption so navigation matches the actual launch contract: locale-prefixed home routes are compatibility redirects too, not independent localized destinations.
- **Files/components changed:** `lib/routes.ts`, `components/layout/Header.tsx`, `scripts/locale-routing-tests.mjs`.
- **User-visible effect:** the OpenGamer logo/home destination and all navigation helpers now resolve directly to the canonical `/` launch route instead of manufacturing `/ru`, `/hy`, `/es` or `/pt` hops that immediately redirect; non-home canonical routing from the previous batch is preserved.
- **Technical rationale:** coordinated QA branch `design/locale-canonical-integrity-polish-20260908-r17` identified that `app/[locale]/page.tsx` permanently redirects every locale-prefixed home to `/`, so the previous handoff statement that localized home destinations were genuine public pages was inaccurate. This batch ports only the focused canonical helper/header/test correction and preserves the implementation branch's stronger current IA labels and other verified changes.
- **Verification:** source parity checked against QA commits `01fbad766eb9700a95f1030b5a86a2066c69905a` and `fa009454c90236f7c012e9037da9873ca42f0852`. Vercel status for final implementation head `991a54b` is **failure due to build-rate limit** (`upgradeToPro=build-rate-limit`), not a reported application/test failure. Local clone/test fallback was attempted but the execution container could not resolve `github.com`, so no fresh local lint/typecheck/test/build success is claimed.
...
- **Rollback:** revert `991a54bcfbc8477ee3092e55ff8e03f62ec6a3c7`, `3a2845dcc0155ff562511b1ee50c20362cd28bde`, then `eb3faceafd758fcc2b7cbfafb74822c603b17117`.
- **CTO production action required:** when Vercel build capacity resumes, run `pnpm lint`, `pnpm typecheck`, `pnpm test`, `pnpm build`; smoke `/ru`, `/hy`, `/es`, `/pt` and confirm each redirects to `/`; verify Header logo/home, desktop/mobile navigation and footer links point directly to canonical launch routes; merge/port only after preview passes.
...
- **CTO production action required:** use the 23:21 correction above as the current locale-home contract; when Vercel build capacity resumes, run the standard verification suite and preview canonical routing before merge/port.
