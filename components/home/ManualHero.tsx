...
export function ManualHero() {
  return (
    <section className="relative isolate overflow-hidden border-b border-white/10 bg-[#05070a]">
      ...
      <Container className="grid gap-12 py-12 sm:py-16 lg:min-h-[calc(100svh-5rem)] lg:grid-cols-[0.74fr_1.26fr] lg:items-center lg:gap-9 lg:py-12 xl:grid-cols-[0.70fr_1.30fr] xl:gap-12 2xl:grid-cols-[0.68fr_1.32fr]">
        <div className="relative z-20 max-w-[42rem] lg:pr-1">
        ...
        <div className="relative z-10 lg:min-h-[690px] xl:min-h-[735px] 2xl:min-h-[780px]" aria-label="Selected OpenGamer games and original product work">
          <div aria-hidden="true" className="absolute inset-x-[10%] top-[6%] hidden h-[68%] rounded-[4rem] bg-emerald/[0.04] blur-3xl lg:block" />
          <div className="grid gap-4 sm:grid-cols-[1fr_0.42fr] sm:items-stretch lg:block">
            <Link href="/portfolio/elementals" className="group relative block aspect-[16/10] overflow-hidden rounded-[1.5rem] border border-white/15 bg-black/45 shadow-[0_30px_100px_rgba(0,0,0,0.44)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 lg:absolute lg:left-0 lg:top-[3%] lg:h-[57%] lg:w-[71%] lg:aspect-auto lg:rounded-[1.75rem] lg:shadow-[0_42px_140px_rgba(0,0,0,0.52)] xl:w-[72%]">
              <Image src="/assets/projects/elementals/expositions/nexus-studio-wheel.webp" alt="ELEMENTALS Nexus studio wheel concept" fill priority sizes="(min-width:1536px) 46vw,(min-width:1280px) 45vw,(min-width:1024px) 44vw,100vw" className="object-cover transition duration-700 group-hover:scale-[1.015] motion-reduce:transition-none motion-reduce:group-hover:scale-100" />
            ...
            <Link href="/portfolio/lc-app" className="group relative mx-auto block aspect-[9/16] w-full max-w-[16rem] overflow-hidden rounded-[1.55rem] border border-white/15 bg-[#030708] shadow-[0_30px_100px_rgba(0,0,0,0.48)] transition duration-300 hover:-translate-y-1 hover:border-emerald/35 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 motion-reduce:transform-none motion-reduce:transition-none sm:max-w-none lg:absolute lg:right-0 lg:top-[7%] lg:h-[68%] lg:w-[27%] lg:aspect-auto lg:rounded-[1.9rem] lg:shadow-[0_42px_130px_rgba(0,0,0,0.62)] xl:w-[26%]">
              ...
              <Image src="/assets/projects/lc-app/optimized/lc-app-mobile-discover.webp" alt="LC App mobile product concept" fill sizes="(min-width:1536px) 17vw,(min-width:1024px) 18vw,(min-width:640px) 28vw,70vw" className="object-contain object-center p-2.5 transition duration-500 group-hover:scale-[1.01] motion-reduce:transition-none motion-reduce:group-hover:scale-100 sm:p-3 lg:p-2.5 xl:p-3" />
          ...
          <div className="mt-5 lg:absolute lg:bottom-[1%] lg:left-0 lg:right-0 lg:mt-0">
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6 lg:gap-2.5" aria-label="Selected playable and portfolio game titles">
              {gameShowcase.map(({ game, image }) => {
                ...
                return <Link key={game.slug} href={`/games/${game.slug}`} className="group overflow-hidden rounded-xl border border-white/12 bg-[#06080b] shadow-[0_18px_48px_rgba(0,0,0,0.38)] transition duration-300 hover:-translate-y-1 hover:border-emerald/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 motion-reduce:transform-none motion-reduce:transition-none"><div className="relative aspect-[10/7] overflow-hidden"><Image src={image} alt={`${game.title} artwork`} fill sizes="(min-width:1536px) 8vw,(min-width:1024px) 9vw,(min-width:640px) 30vw,48vw" className="object-cover transition duration-500 group-hover:scale-[1.03] motion-reduce:transition-none motion-reduce:group-hover:scale-100" /><div className="absolute inset-0 bg-gradient-to-t from-black/92 via-black/12 to-transparent" /><div className="absolute inset-x-0 bottom-0 p-2.5 sm:p-3 lg:p-2.5 xl:p-3"><span className={`block text-[0.46rem] font-semibold uppercase tracking-[0.13em] sm:text-[0.5rem] ${playable ? "text-emerald/90" : "text-slate-300"}`}>{getHomepageStatus(game)}</span><strong className="mt-0.5 block text-[0.72rem] font-semibold leading-tight text-white sm:text-xs lg:text-[0.68rem] xl:text-xs">{game.title}</strong></div></div></Link>;
              })}
            </div>
          </div>
        </div>
