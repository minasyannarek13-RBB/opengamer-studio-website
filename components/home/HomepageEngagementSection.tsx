...
const capabilities = [
  ...
  },
  {
    ...
    proof: [{ src: "/assets/projects/elementals/expositions/nexus-stage.webp", alt: "ELEMENTALS original Live Casino IP concept" }]
  }
];
            ...
            {capabilities.map((item) => (
              <Link key={item.number} href={item.href} className="group relative grid gap-5 border-b border-white/10 py-8 transition duration-300 hover:border-emerald/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 motion-reduce:transition-none sm:grid-cols-[3.2rem_1fr_auto] sm:gap-6 sm:py-10 lg:grid-cols-[3.5rem_1fr_auto]">
                ...
                <div className="min-w-0">
                  <p className="text-[0.66rem] font-semibold uppercase tracking-[0.18em] text-slate-500 transition group-hover:text-emerald/80 motion-reduce:transition-none">{item.kicker}</p>
                ...
                <div className="hidden items-center sm:flex"><span aria-hidden="true" className="flex h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/[0.025] text-slate-500 transition duration-300 group-hover:translate-x-1 group-hover:border-emerald/30 group-hover:bg-emerald/[0.06] group-hover:text-emerald motion-reduce:transform-none motion-reduce:transition-none">→</span></div>
                <div aria-hidden="true" className="pointer-events-none absolute inset-y-0 left-0 w-px origin-top scale-y-0 bg-gradient-to-b from-emerald/0 via-emerald/45 to-emerald/0 transition-transform duration-500 group-hover:scale-y-100 motion-reduce:hidden" />
              </Link>
            ))}
          </div>
        </div>
