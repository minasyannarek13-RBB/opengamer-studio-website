...
export function ServiceCard({ service }: { service: ServiceItem }) {
  return (
    <article className="premium-card rounded-2xl border border-line bg-white/[0.045] p-5 shadow-[0_18px_60px_rgba(0,0,0,0.2)] sm:p-6">
      ...
      <ul className="mt-6 grid gap-2.5 sm:grid-cols-2">
        {service.capabilities.map((capability) => (
          <li key={capability} className="flex gap-2.5 rounded-lg border border-white/[0.06] bg-black/15 px-3 py-2 text-sm text-slate-300">
