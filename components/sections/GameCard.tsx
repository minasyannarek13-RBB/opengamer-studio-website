  ...
  return (
    <article className="premium-card game-card group flex h-full min-w-0 flex-col overflow-hidden rounded-[var(--radius-card)] border border-white/10 bg-white/[0.035] shadow-[0_18px_60px_rgba(0,0,0,0.20)] transition duration-300 hover:-translate-y-0.5 hover:border-emerald/25 hover:bg-white/[0.05] hover:shadow-[0_24px_76px_rgba(0,0,0,0.26)] focus-within:-translate-y-0.5 focus-within:border-emerald/45" style={{ "--game-accent": game.visualAccent || "#2ee6a6" } as CSSProperties}>
      <Link href={`/games/${game.slug}`} className="image-frame relative aspect-[10/7] overflow-hidden bg-black/45 focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald/70">
        ...
        <div className="absolute inset-x-0 bottom-0 flex min-w-0 flex-wrap items-end justify-between gap-2 p-3 sm:p-4">
          {primaryCategory ? <span className="max-w-[72%] break-words rounded-full border border-white/15 bg-black/55 px-3 py-1 text-xs font-medium leading-4 text-white backdrop-blur">{primaryCategory}</span> : <span />}
      ...
      <div className="flex min-w-0 flex-1 flex-col p-5 sm:p-6">
        <h3 className="break-words text-xl font-semibold leading-tight text-white sm:text-2xl">{game.title}</h3>
        {commercialStatus ? <p className="mt-1 break-words text-xs text-slate-500">{commercialStatus}</p> : null}
        <p className="mt-3 break-words text-sm leading-6 text-slate-400">{game.shortDescription}</p>
        <div className="mt-auto flex min-w-0 flex-col gap-3 pt-6 min-[420px]:flex-row min-[420px]:flex-wrap min-[420px]:items-center">
          {demoUrl ? <Button href={demoUrl} className="min-h-10 w-full px-4 min-[420px]:w-auto" target="_blank" rel="noopener noreferrer">Play Demo</Button> : <Button href={`/contact?interest=game&game=${game.slug}#project-enquiry`} variant="secondary" className="min-h-10 w-full px-4 min-[420px]:w-auto">Request Demo</Button>}
          <Button href={`/games/${game.slug}`} variant="link" aria-label={`View details for ${game.title}`} className="w-fit">View Game</Button>
        </div>
      </div>
    </article>
  );
