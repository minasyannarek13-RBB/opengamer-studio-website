...
export function ProductSignature({ eyebrow, title, description, image, href, actionLabel, accent = "#2ee6a6", compact = false }: ProductSignatureProps) {
  return (
    <Link
      href={href}
      className={`group relative isolate block overflow-hidden rounded-[1.2rem] border border-white/10 bg-[#06090b] transition duration-300 hover:-translate-y-0.5 hover:border-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 motion-reduce:transform-none motion-reduce:transition-none ${compact ? "p-3" : "p-4 sm:p-5"}`}
    ...
    >
      ...
      <div className={`relative grid items-center ${compact ? "grid-cols-[5.2rem_1fr] gap-3.5" : "grid-cols-[6.5rem_1fr] gap-4 sm:grid-cols-[7.4rem_1fr] sm:gap-5"}`}>
        <div className="relative aspect-[10/8] overflow-hidden rounded-xl border border-white/[0.08] bg-black/40">
          <Image src={image} alt={`${title} visual reference`} fill sizes={compact ? "84px" : "120px"} className="object-cover transition duration-500 group-hover:scale-[1.025] motion-reduce:transition-none motion-reduce:group-hover:scale-100" />
...
export function RelatedProductStrip({ items }: { items: ProductSignatureProps[] }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3" data-reveal-group="cards">
