...
test("localized homepage remains localized while launch pages stay canonical", async () => {
  const { getLocalizedHomePath } = await cacheSafeImport("../lib/routes.ts");

  assert.equal(getLocalizedHomePath("ru", "/"), "/ru");
  assert.equal(getLocalizedHomePath("hy", "/"), "/hy");
...
test("header and footer CTAs do not manufacture localized redirect URLs", async () => {
