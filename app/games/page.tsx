          ...
          <div className="grid gap-3 lg:grid-cols-12 lg:grid-rows-2" aria-label="Selected OpenGamer game titles">
            {featuredTitles.map((game, index) => (
              <Link
                ...
                className={`group relative min-h-[220px] overflow-hidden rounded-[1.35rem] border border-white/10 bg-black/40 shadow-[0_24px_70px_rgba(0,0,0,0.25)] transition duration-300 hover:-translate-y-0.5 hover:border-emerald/30 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70 ${index === 0 ? "lg:col-span-7 lg:row-span-2 sm:min-h-[380px] lg:min-h-[470px]" : "lg:col-span-5 sm:min-h-[250px]"}`}
              >
                <Image src={game.artwork?.catalogue || game.image} alt={`${game.title} artwork`} fill priority={index === 0} sizes={index === 0 ? "(min-width:1024px) 55vw,100vw" : "(min-width:1024px) 40vw,100vw"} className="object-cover transition duration-700 group-hover:scale-[1.025]" />
          ...
          <Container className="relative">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl font-semibold tracking-[-0.015em] text-white sm:text-3xl">Confirmed catalogue titles without a public demo</h2>
              </div>
              <p className="max-w-xl text-sm leading-6 text-slate-500">These entries remain visible as confirmed portfolio work. No demo, launch, certification or commercial-use claim is implied.</p>
            </div>

            <div className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {portfolioHighlights.map((game) => (
                <Link key={game.slug} href={`/games/${game.slug}`} className="group relative overflow-hidden rounded-[1.15rem] border border-white/10 bg-white/[0.025] p-3 transition duration-300 hover:-translate-y-0.5 hover:border-emerald/25 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70">
                  <div className="relative aspect-[10/7] overflow-hidden rounded-[0.9rem] border border-white/[0.06] bg-black/30">
                    <Image src={game.artwork?.catalogue || game.image} alt={`${game.title} artwork`} fill sizes="(min-width:1024px) 22vw,(min-width:640px) 46vw,100vw" className="object-cover transition duration-500 group-hover:scale-[1.025]" />
