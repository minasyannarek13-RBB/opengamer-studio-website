...
export default function TechnologyPage() {
  return (
    <SiteShell atmosphere="technology">
          ...
          <div className="relative">
            ...
            {engineeringScope.map(([index, title, description, entryPoint], itemIndex) => (
              <div key={title} className="group relative grid gap-4 border-b border-white/10 py-7 pl-12 transition duration-300 first:border-t sm:grid-cols-[0.9fr_1.1fr] sm:gap-8 sm:py-8 sm:pl-16">
                ...
                <div><span className="text-[0.58rem] font-semibold uppercase tracking-[0.16em] text-slate-500">{entryPoint}</span><h2 className="mt-2 text-xl font-semibold text-white transition group-hover:text-emerald sm:text-2xl">{title}</h2></div>
        ...
        <div className="relative grid gap-12 lg:grid-cols-[0.58fr_1.42fr] lg:gap-16 xl:gap-24">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {integrationSteps.map(([number, title, text, phase]) => (
              <div key={number} className="group relative min-h-[180px] overflow-hidden rounded-[1.15rem] border border-white/10 bg-white/[0.025] p-5 transition duration-300 hover:border-emerald/25 sm:p-6">
                ...
                <h3 className="relative mt-6 text-xl font-semibold text-white transition group-hover:text-emerald">{title}</h3>
      ...
      <CTASection title="Define the Technical Boundary First" description="Share the target environment, current implementation, technical dependencies and the gap you want OpenGamer to own or support." ctaLabel="Discuss Integration" ctaHref="/contact?service=technology#project-enquiry" secondaryLabel="View Development Services" secondaryHref="/services" />
    </SiteShell>
  );
}
