    ...
    <SiteShell atmosphere="company">
          ...
          <div className="grid gap-12 lg:grid-cols-[0.68fr_1.32fr] lg:items-center lg:gap-16 xl:gap-20">
            <div>
              <SectionHeader
                ...
                title="Games, Original IP and Product Work You Can Inspect"
                description="A portfolio across playable slot production, original Live Casino IP and B2B product-interface work, with status kept deliberately clear."
                headingLevel="h1"
              />
              ...
              <div className="mt-8 flex flex-wrap gap-2" aria-label="Portfolio proof types">
                {["Playable games", "Original IP", "Product interfaces", "Commercial scope"].map((item) => (
            ...
            <div className="relative min-h-[620px] sm:min-h-[690px] lg:min-h-[670px]" aria-label="Selected OpenGamer portfolio work">
              ...
              <Link href="/portfolio/elementals" className="group absolute left-0 top-0 h-[60%] w-[72%] overflow-hidden rounded-[1.6rem] border border-white/15 bg-black/45 shadow-[0_34px_110px_rgba(0,0,0,0.42)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70">
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 sm:p-7">
                  ...
                  <p className="mt-2 max-w-lg text-sm leading-6 text-slate-300">Original show-game product thinking built around a cinematic wheel, four elemental realms and a host-led experience.</p>
                </div>
              </Link>
              ...
              <Link href="/portfolio/lc-app" className="group absolute right-0 top-[7%] h-[53%] w-[31%] overflow-hidden rounded-[1.5rem] border border-white/15 bg-[#06090b] shadow-[0_30px_95px_rgba(0,0,0,0.42)] focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70">
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/92 via-black/65 to-transparent p-4 sm:p-5">
              ...
              <div className="absolute inset-x-0 bottom-0">
                <div className="mb-3 flex items-end justify-between gap-4">
                  <div><p className="text-[0.58rem] font-semibold uppercase tracking-[0.17em] text-slate-500">Game production proof</p><p className="mt-1 text-sm text-slate-400">Playable and portfolio titles from the OpenGamer catalogue.</p></div>
                  <Link href="/games" className="hidden text-sm font-semibold text-white/75 transition hover:text-emerald sm:inline-flex">View all games →</Link>
                </div>
                  ...
                  {gameProof.map((game) => (
                    <Link key={game.slug} href={`/games/${game.slug}`} className="group/game relative overflow-hidden rounded-xl border border-white/10 bg-black/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald/70">
                      <div className="relative aspect-[10/7] overflow-hidden">
                        <Image src={game.image} alt={`${game.title} artwork`} fill sizes="(min-width:1024px) 14vw,(min-width:640px) 23vw,48vw" className="object-cover transition duration-500 group-hover/game:scale-[1.025]" />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/88 via-transparent to-transparent" />
                        <div className="absolute inset-x-0 bottom-0 p-3"><span className="text-[0.48rem] font-semibold uppercase tracking-[0.12em] text-emerald">{game.status}</span><strong className="mt-0.5 block text-xs text-white sm:text-sm">{game.title}</strong></div>
                      </div>
                    </Link>
                  ))}
                </div>
      ...
      <Section className="relative overflow-hidden">
        ...
        <div className="relative grid gap-12 lg:grid-cols-[0.58fr_1.42fr] lg:gap-16 xl:gap-24">
          <div className="lg:sticky lg:top-28 lg:self-start">
            <SectionHeader eyebrow="What the portfolio proves" title="Different Proof for Different Buyer Questions" description="A playable game, an original IP and a B2B product concept prove different things. The portfolio keeps those proof types separate instead of pretending they are interchangeable." />
          </div>
      ...
      <Section>
        <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
          <SectionHeader eyebrow="Casino games" title="Selected Game Production Proof" description="A curated sample of the OpenGamer slot portfolio. Public demos are shown where verified; portfolio-only titles remain visible without pretending a demo exists." />
      ...
      <CTASection
        ...
        description="Discuss a playable title, original concept, portfolio adaptation or new production brief with OpenGamer."
